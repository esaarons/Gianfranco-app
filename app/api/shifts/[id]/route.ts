...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Compute summary from area_cards and orders in this shift window
  ...
  // Overall card metrics
  const reactionTimes = cards
    .filter(c => c.received_at)
    .map(c => diffMin(c.created_at, c.received_at))

  const prepTimes = cards
    .filter(c => c.received_at && c.delivered_at)
    .map(c => diffMin(c.received_at, c.delivered_at))

  const totalTimes = cards
    .filter(c => c.delivered_at)
    .map(c => diffMin(c.created_at, c.delivered_at))
  ...
  const cycleTimes = orders
    ...
    .map(o => diffMin(o.created_at, o.closed_at))
  ...
  const { data, error } = await supabase
    .from('shifts')
    .update({ ended_at: endedAt, ended_by: userId, summary })
    .eq('id', id)
    .select()
    .single()
