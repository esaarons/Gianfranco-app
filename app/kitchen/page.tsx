...
const KITCHEN_AREA_ID = AREA_IDS.KITCHEN
...
export default function KitchenPage() {
  const { data: cards = [], isLoading } = useAreaCards(KITCHEN_AREA_ID)
    ...
    <div className="min-h-screen bg-[#F6F2EA]">
      ...
      <div className="px-5 pt-8 pb-3">
        ...
        <div className="flex items-center justify-between">
          <h1 className="text-[#252525] text-2xl font-bold tracking-tight">Cocina</h1>
          {pending.length > 0 && (
            <span className="flex items-center gap-1.5 bg-[#C76868]/10 text-[#8B3A3A] text-xs font-bold px-3 py-1.5 rounded-full border border-[#C76868]/20">
              <span className="w-1.5 h-1.5 rounded-full bg-[#C76868] dot-pulse-rust" />
            ...
            >
              ...
              {counts[t.key] > 0 && (
                <span className={cn(
                  ...
                  tab === t.key
                    ? t.key === 'pending' ? 'bg-[#C76868] text-white' : 'bg-white/20 text-white'
                    : 'bg-[#C76868]/12 text-[#8B3A3A]'
                )}>
                  {counts[t.key]}
                </span>
              )}
